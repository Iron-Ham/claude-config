...
3. **Use `$impeccable teach` (REQUIRED)**: If neither source has context, you MUST use `$impeccable teach` NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
...
Then STOP and ask the user whether they'd also like the Design Context appended to AGENTS.md. If yes, append or update the section there as well.
