...
Use `$impeccable`, which contains design principles, anti-patterns, and the **Context Gathering Protocol**. Follow the protocol before proceeding. If no design context exists yet, you MUST use `$impeccable teach` first. Additionally gather: what the interface is trying to accomplish.
...
**After presenting findings**, use targeted questions based on what was actually found. STOP and ask the user to clarify. These answers will shape the action plan.
